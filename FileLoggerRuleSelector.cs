...
{
    internal class FileLoggerRuleSelector
    {
        public void Select(FileLoggerFilterOptions options, Type providerType, string category, out LogType logType,
            out LogLevel? minLevel, out Func<string, string, LogLevel, bool> filter)
        {
            ...
            var providerAlias = FileLoggerProviderAliasUtilities.GetAlias(providerType);
            FileLoggerFilterRule current = null;
        ...
        private static bool IsBetter(FileLoggerFilterRule rule, FileLoggerFilterRule current, string logger, string category)
        {
